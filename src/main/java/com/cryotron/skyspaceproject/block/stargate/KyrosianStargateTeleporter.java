...
public class KyrosianStargateTeleporter implements ITeleporter {
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("kyrosian_stargate", Vec3i::compareTo, 300);
	...
	public KyrosianStargateTeleporter(ServerLevel worldServer, BlockPos entityEnterPos) {
	...
	private PortalInfo getPortalInfo(Entity entity, ServerLevel server) {
		...
		return this.getExitPortal(entity, blockpos1, worldborder).map(repositioner -> {
			...
			if (blockstate.hasProperty(BlockStateProperties.HORIZONTAL_AXIS)) {
				...
				vector3d = KyrosianStargateShape.getRelativePosition(teleportationrepositioner$result, direction$axis, entity.position(),
						entity.getDimensions(entity.getPose()));
			} else {
				direction$axis = Direction.Axis.X;
				vector3d = new Vec3(0.5, 0, 0);
			}
			return KyrosianStargateShape.createPortalInfo(server, repositioner, direction$axis, vector3d, entity.getDimensions(entity.getPose()),
	...
	protected Optional<BlockUtil.FoundRectangle> getExitPortal(Entity entity, BlockPos pos, WorldBorder worldBorder) {
		Optional<BlockUtil.FoundRectangle> optional = this.findPortalAround(new BlockPos(0,4,0), false, worldBorder);
...
//						.orElse(Direction.Axis.X);
				return optional;  //this.createPortal(pos, direction$axis);
			}
		} else {
			return optional;
		}
